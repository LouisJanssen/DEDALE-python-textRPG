...
def OdysseusDialogue() :
    ...
    if OdysseusAnswer == '1' :
        ...
        promptSlow('- Crois-moi, je le ferais, si je le pouvais. Seulement, ma présence ici est temporaire. Zeus est parvenu à convaincre Morphée de me laisser te contacter. Il sait se montrer persuasif... Enfin, tu ne vas pas tarder à te réveiller, alors je dois faire vite, le temps presse. Tu dois trouver la source vitale du labyrinthe ! Dédale a dû choisir une créature assez puissante pour le supporter. Probablement une créature liée à son passé, je le crains... En revanche, il faudrait une créature ayant à la fois une force physique et mentale hors du commun. Mais qui ? Quoi ? C\'est à toi de le découvrir, je suppose. Une fois que tu feras face à cette source, tu devras la détruire pour que tout revienne dans l\'ordre. C\'est le seul moyen d\'en finir avec cette folie.')
        OdysseusDialogue()
