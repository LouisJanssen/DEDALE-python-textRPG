...
# SAVE PLAYER STATS (before combats)
...
def ObjectInventory(objectName):
  ...
  if str(objectName) in InventoryList:
    ...
    print('Où souhaitez vous placer l\'objet ? 1, 2, 3, 4 ou 5 ?')
    print(InventoryList)
    ask = input(' > ')
    print('')
...
def LevelUp(XP, amount): # Increases Player XP depending on which enemy he fought : if he has enough XP, his level is increased
    XP += amount
    Player.xp += XP
    if Player.xp >= 25 :
        PlayerLevel = 2
